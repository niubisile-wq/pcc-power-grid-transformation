...
ASSETS = ROOT / "results" / "stage5_import_assets"
WORKER_RESULTS = ROOT / "logs" / "stage5_import_workers"
CONSOLE_LOGS = ROOT / "logs" / "stage5_import_console"
...
def _row_fields(row: dict[str, str]) -> dict[str, str]:
    return {
        key: row[key]
        for key in (
            "case_id",
...
def _run_attempt(
    ...
    if resume and result_output.is_file():
        ...
        return {
            **_row_fields(row),
            ...
            "console_log_path": (
                console_output.relative_to(ROOT).as_posix()
                if console_output.is_file()
                else ""
            ),
            "resumed": True,
        }
    ...
    return {
        **_row_fields(row),
        ...
        "result_path": (
            result_output.relative_to(ROOT).as_posix()
...
def main() -> None:
    ...
    output = ROOT / "results" / "stage5_import_matrix_results.csv"
    for model_index, row in enumerate(models, 1):
        for tool in TOOLS:
            ...
            print(
                f"  {result['status']} assets={result['asset_count']} "
                f"elapsed={float(result['elapsed_seconds']):.2f}s "
                f"peak={result['peak_rss_mb']}",
                flush=True,
            )
            pd.DataFrame(attempts).to_csv(output, index=False)
    frame = pd.DataFrame(attempts)
    status_table = (
        frame.groupby(["tool", "status"], dropna=False)
        .size()
        .reset_index(name="count")
    )
    status_table.to_csv(
        ROOT / "results" / "stage5_import_matrix_status_by_tool.csv", index=False
    )
    latency = (
    ...
    latency.to_csv(
        ROOT / "results" / "stage5_import_latency_summary.csv", index=False
    )
    summary = {
    ...
    (ROOT / "results" / "stage5_import_matrix_summary.json").write_text(
