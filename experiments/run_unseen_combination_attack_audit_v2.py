"""Expanded unseen-combination audit for the fail-closed certificate verifier.

This extends the original combination set with additional multi-field and
multi-constraint attack chains so the protocol-level evidence is denser.
"""
...
DATE = "20260803"
OUT_CSV = ROOT / f"unseen_combination_attack_v2_results_{DATE}.csv"
OUT_JSON = ROOT / f"unseen_combination_attack_v2_summary_{DATE}.json"
N_PER = 1000
...
ATTACKS = [
    ...
    ("combo_provenance_signature", ["missing_provenance", "signature_tamper"]),
    ("combo_authorization_chain", ["missing_authorization", "chain_append"]),
    ("combo_target_version", ["target_rewrite", "version_downgrade"]),
    ("combo_unknown_signature_chain", ["unknown_source", "signature_strip", "chain_append"]),
]
...
def main():
    ...
    for index, (name, mutations) in enumerate(ATTACKS, start=1):
        for repeat in range(N_PER):
            ...
            rows.append({
                "attack_id": f"U2{index:02d}",
                "run_id": f"U2{index:02d}-{repeat:04d}",
                "attack_class": name,
    ...
    summary = {
        "experiment": "unseen_combination_attacks_v2",
        "date": DATE,
