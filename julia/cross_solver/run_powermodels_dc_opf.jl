...
if length(ARGS) != 2
    error("usage: run_powermodels_dc_opf.jl INPUT_TSV OUTPUT_TSV")
end
...
open(output_path, "w") do output
    println(output, join([
        ...
        "generator_dispatch_mw", "solve_time_s", "julia_version", "powermodels_version", "ipopt_version"
    ], '\t'))
    for line in eachline(input_path)
        ...
        generator_dispatch = join(
            [string(id, "=", base_mva * get(generator, "pg", 0.0)) for (id, generator) in sort(collect(get(result["solution"], "gen", Dict())); by=first)],
            ";",
        )
        println(output, join([
            network,
